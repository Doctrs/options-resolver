...
class OptionsResolver implements Options
{
    ...
    public function setDefault($option, $value)
    {
        ...
        if (\is_array($value)) {
            ...
            $optionsResolver->setNestedLevel(
                array_merge(
                    $this->getNestedLevel(),
                    [$option]
                )
    ...
    public function hasDefault($option)
    {
        if (\is_array($option)) {
            ...
            return $this->nested[$nestedOption]->hasDefault(
                1 === count($option) ?
                    $option[0] : $option
            );
        }
    ...
    public function setRequired($optionNames)
    {
        ...
        foreach ((array) $optionNames as $option) {
            if (\is_array($option)) {
                $nestedOption = array_shift($option);
                if (array_key_exists($nestedOption, $this->nested)) {
                    $this->nested[$nestedOption]->setRequired(
                        1 === count($option) ?
                            $option : [$option]
                    );
                }
    ...
    public function isRequired($option)
    {
        if (\is_array($option)) {
            ...
            return $this->nested[$nestedOption]->isRequired(
                1 === count($option) ?
                    $option[0] : $option
            );
        }
    ...
    public function isMissing($option)
    {
        ...
        if (\is_array($option)) {
            ...
            return $this->nested[$nestedOption]->isMissing(
                1 === count($option) ?
                    $option[0] : $option
            );
        }
    ...
    public function setDefined($optionNames)
    {
        ...
        foreach ((array) $optionNames as $option) {
            if (\is_array($option)) {
                $nestedOption = array_shift($option);
                if (array_key_exists($nestedOption, $this->nested)) {
                    $this->nested[$nestedOption]->setDefined(
                        1 === count($option) ?
                            $option : array($option)
                    );
                }
    ...
    public function isDefined($option)
    {
        if (\is_array($option)) {
            ...
            return $this->nested[$nestedOption]->isDefined(
                1 === count($option) ?
                    $option[0] : $option
            );
        }
    ...
    public function setAllowedValues($option, $allowedValues)
    {
        ...
        if (\is_array($option)) {
            $nestedOption = array_shift($option);
            if (array_key_exists($nestedOption, $this->nested)) {
                $this->nested[$nestedOption]->setAllowedValues(
                    1 === count($option) ?
                        $option[0] : $option,
                    $allowedValues
                );
    ...
    public function addAllowedValues($option, $allowedValues)
    {
        ...
        if (\is_array($option)) {
            $nestedOption = array_shift($option);
            if (array_key_exists($nestedOption, $this->nested)) {
                $this->nested[$nestedOption]->addAllowedValues(
                    1 === count($option) ?
                        $option[0] : $option,
                    $allowedValues
                );
    ...
    public function setAllowedTypes($option, $allowedTypes)
    {
        ...
        if (\is_array($option)) {
            $nestedOption = array_shift($option);
            if (array_key_exists($nestedOption, $this->nested)) {
                $this->nested[$nestedOption]->setAllowedTypes(
                    1 === count($option) ?
                        $option[0] : $option,
                    $allowedTypes
                );
    ...
    public function addAllowedTypes($option, $allowedTypes)
    {
        ...
        if (\is_array($option)) {
            $nestedOption = array_shift($option);
            if (array_key_exists($nestedOption, $this->nested)) {
                $this->nested[$nestedOption]->addAllowedTypes(
                    1 === count($option) ?
                        $option[0] : $option,
                    $allowedTypes
                );
    ...
    public function remove($optionNames)
    {
        ...
        foreach ((array) $optionNames as $option) {
            if (\is_array($option)) {
                $nestedOption = array_shift($option);
                if (array_key_exists($nestedOption, $this->nested)) {
                    $this->nested[$nestedOption]->remove(
                        1 === count($option) ?
                            $option : array($option)
                    );
                }
    ...
    public function resolve(array $options = array())
    {
        ...
        if (count($diff) > 0) {
            ...
            $keys = array_map(function ($item) use ($clone) {
                return implode(':', $clone->getNestedLevel()) . ':' . $item;
            }, array_keys($diff));
    ...
    public function offsetGet($option)
    {
        ...
        if (\is_array($option)) {
            $nestedOption = array_shift($option);
            if (!isset($this->nested[$nestedOption])) {
                throw new NoSuchOptionException(sprintf(
                    'The nested option "%s" has no value set.',
                    $nestedOption
                ));
            }

            return $this->nested[$nestedOption]->offsetGet(
                1 === count($option) ?
                    $option[0] : $option
            );
        }
     ...
     * @return array
     */
    public function getNestedLevel(): array
    {
     ...
     * @return self
     */
    public function setNestedLevel(array $nestedLevel): self
    {
    ...
    /**
     * Вызов происходит только в замыканиях и родитель не всегда заблокирован
     * Блокируем его принудительно
     *
     * @return null
     */
    ...
    {
        if (!$this->parent) {
            throw new AccessException('You are in first level OptionResolver');
        }
